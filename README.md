...
| [Chip&#8209;Design](https://autochip-lab.github.io/Knowledge-Hub/Chip-Design/) | 以中英双语梳理芯片从产品定义、架构、RTL、验证、综合、物理设计、签核到量产的完整流程，并提供 DFT 可测性设计专题。 |
...
文档支持中文与英文切换。在十三个编号主阶段之外，还增加了贯穿架构、RTL、综合、物理设计、签核、ATE 测试和量产诊断的 **DFT（Design for Test，可测性设计）专题**，帮助读者理解制造测试能力为什么必须从设计早期开始规划。
...
- DFT / test agent 为什么需要跨越设计、实现、签核和量产多个阶段；
...
这份文档把十三个主流程环节压缩成五个连续阶段，同时把 DFT 作为跨阶段专题单独展开：
...
| 跨阶段专题 | DFT 可测性设计 | 从架构规划、scan / MBIST / ATPG 到物理实现、ATE 测试、失效诊断和良率学习。 |

完整顺序：`① 产品定义` -> `② 系统规格` -> `③ 架构` -> `④ 微架构` -> `⑤ RTL` -> `⑥ 验证` -> `⑦ 综合` -> `⑧ 物理设计` -> `⑨ 签核` -> `⑩ 流片` -> `⑪ 制造` -> `⑫ 封测` -> `⑬ bring-up / 量产`

DFT 不作为第十四个串行阶段，而是与多个主阶段协同推进的贯穿性能力。
...
| DFT 可测性设计 | 建立 scan、compression、MBIST / BISR、JTAG / IJTAG、ATPG、ATE 与诊断闭环。 | 帮助 DFT / test agent 理解跨阶段约束、交付物、签核与量产反馈。 |
...
| DFT / Test Agent | DFT 专题、③、④、⑤、⑦、⑧、⑨、⑫、⑬ |
| Tapeout / Bring-up Assistant | ⑨、⑩、⑪、⑫、⑬ |

#### 这份文档的特点

- **网页化**：可以直接通过浏览器访问，不需要本地环境。
- **中英双语**：中文与英文内容可以在网页中直接切换。
- **流程化**：按芯片从想法到量产的真实顺序组织。
- **包含 DFT 专题**：把可测性设计作为跨阶段工程能力单独展开。
