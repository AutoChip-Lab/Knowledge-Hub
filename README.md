...
| [Self-Evolving Agents](https://autochip-lab.github.io/Knowledge-Hub/Agent-Self-Evolving/) | 以时间线梳理自演化 agent 的代表性研究、核心机制、公式、实验结果、实现模式与现实局限。 | 为 AutoChip agent 的反思、经验积累、技能更新、自动搜索和持续改进提供研究背景。 |
| [Hermes Design Flow](https://autochip-lab.github.io/Knowledge-Hub/Hermes-Design-Flow/) | 从源码路径拆解 Hermes Agent 的核心架构、执行闭环、memory 与 skill 更新、工具与子代理、会话存储和搜索。 | 提供一个可落到代码级别的 agent harness、runtime、context 和持久化参考实现。 |
| [Multi-Agent Knowledge](https://autochip-lab.github.io/Knowledge-Hub/Multi-Agent-Knowledge/) | 以中英双语系统讲解角色与团队、任务规划、协作拓扑、通信协议、路由交接、评测调试、安全生产化和案例。 | 为 AutoChip 多 agent 平台的团队设计、协议选择、可观测性、可靠性和生产部署提供深入参考。 |
...
| 负责 AutoChip memory、skill 或经验复用的成员 | 理解反思、记忆更新、技能沉淀与策略搜索的不同机制。 |
...
| AutoChip 需求 | 这份文档提供的支撑 |
| --- | --- |
| 从芯片设计任务中持续积累经验 | 区分短期反思、长期记忆、技能库和策略更新的作用。 |
| 自动改进 agent workflow 与工具使用 | 提供程序搜索、工作流优化和能力归档的研究参照。 |
| 防止自修改导致性能退化 | 强调外部验证、基准回归、版本保留和失败恢复的重要性。 |
| 建立长期演进的 AutoChip agent | 帮助团队判断哪些能力适合在线学习，哪些改动必须离线评测后发布。 |
...
这份文档的价值在于把抽象的 agent 概念落到具体代码路径。每一章都对应 Hermes 的真实模块和源码位置，因此既可以用于理解系统架构，也可以作为设计 AutoChip agent harness 与 runtime 时的工程参考。
...
| AutoChip 场景 | 这份文档提供的支撑 |
| --- | --- |
| 构建统一的芯片设计 agent runtime | 提供共享 conversation loop 与模型、工具、状态解耦的参考结构。 |
| 接入 EDA 工具、脚本和外部服务 | 展示工具执行、代码批处理、权限继承和结果回填的实际路径。 |
| 管理长时间芯片设计任务的上下文 | 提供 context compression、持久会话和全文搜索的实现思路。 |
| 发展 memory、skills 与子代理能力 | 展示长期知识更新、后台复盘和受控委派如何嵌入主系统。 |
...
| 读者 | 阅读收益 |
| --- | --- |
| 负责 AutoChip 多 agent 总体架构的成员 | 系统建立角色、任务、拓扑、协议和治理之间的关系。 |
...
| AutoChip 场景 | 这份文档提供的支撑 |
| --- | --- |
| 将芯片全流程拆给多个专门 agent | 提供从任务 DAG 到角色职责、输入输出契约的设计方法。 |
| 在架构、RTL、验证和后端之间传递结果 | 帮助设计结构化消息、handoff 条件、共享状态与反馈闭环。 |
| 根据任务动态选择 agent 与工具 | 比较固定流水线、主管路由、一般图和动态拓扑的适用边界。 |
| 建立可审计、可评测的生产系统 | 提供轨迹观测、失败归因、安全审批和回归评测框架。 |
